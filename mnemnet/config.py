...
class TensionConfig:
    ...
    # Contradictions found by meaning (tension_candidates). The literal check above
    # only compares one predicate with itself; agents that phrase a predicate per
    # thought almost never repeat one. Cosine similarity between two facts about the
    # same subject; calibrated on archived facts of two long-running agents, where a
    # typical same-subject pair scored 0.19 and the 99th percentile 0.53.
    similarity_floor: float = field(default_factory=lambda: float(
        _get("tension", "similarity_floor", "MNEMNET_TENSION_SIMILARITY", 0.6)
    ))
    per_pass: int = field(default_factory=lambda: int(
        _get("tension", "per_pass", "MNEMNET_TENSION_PER_PASS", 4)
    ))
