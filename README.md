...
```python
kg_add_smart("agent", "knows", "Sam")
kg_add_smart("agent", "knows", "Robin")   # no tension — both are true
...
**The exact check is nearly blind in practice.** It compares a predicate only with itself. Agents
that write free-form predicates phrase one per thought: on two long-running agents, 80 distinct
predicates in 82 facts and 162 in 168. Over three months that produced **zero** tensions. One of
them held *"the move was a fresh start"* and, a day later, *"the move was really running
away"* — side by side, never compared.

So there is a second path, by meaning. `tension_candidates()` returns pairs of current facts about
the same subject that are close in meaning; the agent weighs each — ideally in an offline pass such
as a dream — and `weigh_pair()` records the verdict:

```python
from mnemnet import tension_candidates, weigh_pair

for pair in tension_candidates(["agent"]):          # up to 4, most similar first, older fact as "a"
    verdict = ask_the_agent(pair)                    # "tension" | "revised" | "fine"
    weigh_pair(pair, verdict, note="why")
```

- **tension** — both still hold and cannot both be true: kept open as a `_tension_` node.
- **revised** — the later is what the agent thinks now: the earlier is ended (never deleted) and a
  `_corrected_` record says so.
- **fine** — they sit together.

Every verdict is written to the graph, so a pair is not offered twice. Deciding stays with the agent
on purpose — similarity finds candidates, it cannot tell a contradiction from a restatement.

Similarity uses local sentence embeddings (the model chroma ships with, English-trained; less reliable
for other languages). The floor, 0.6, was set against archived facts of two agents — a typical
same-subject pair scored 0.19, the 99th percentile 0.53 — and has **not yet been validated on live
use**; it is configurable (`[tension] similarity_floor`, `MNEMNET_TENSION_SIMILARITY`). Pass your own
`embed=` callable to use a different model.

### 5. Provenance — and one correction path

Every fact can say where it came from:

```python
kg_add_smart("agent", "lives_with", "Sam", source="wanted")
kg_add_smart("agent", "woke_at", "3am",     source="perceived")
kg_add_smart("agent", "believes", "house_is_full", source="inferred")
```
...
The last one is why this exists. A running agent invented a housemate from two empty
placeholder files, recorded it, and the graph handed the invention back as a loud fact
on the next waking — which it then defended. Memory with no correction does not fix an
error, it entrenches it.

So `living_context()` keeps wishes and keeps them apart:

```
◈ agent
  [now] agent —woke_at→ 3am
  [now inferred] agent —believes→ house_is_full
  ◇ wished, not observed: agent —lives_with→ Sam
```

Read back in the same shape as everything else, a wish is indistinguishable from an
observation. Given its own line, it stays a wish.

**Correcting is one call, and it is not a delete:**

```python
correct("agent", "lives_with", "nobody_yet",
        source="perceived",
        because="checked the house state — those were empty placeholder files")
```

Deliberately not a `forget`. Given tools to delete and a contradiction about its own
past choices, a model clears the record and announces a replacement the next step
erases — 42 announced across one study, none delivered, because deleting and writing
were two calls with a gap between them. One call has no gap.

Nothing is destroyed: the old value is ended, stops being current and stops surfacing,
and stays readable in the timeline. Corrections show up in context on their own line —
an agent that can see it has fixed itself before is in a different position from one
for whom every belief has always been true.

> Lin, R., Iskakova, A. & Wofford, T. (2026). *Choosing Not to Choose: Self-Authored
> Contradictions Suppress Arbitration in a Memory-Augmented LLM.*
...
Configure in `~/.mnemnet/config.toml`. See `schemas/example.toml` for a full example.
...
Copy `schemas/example.toml` to `~/.mnemnet/config.toml` and edit:
...
[tension]
single_valued    = ["mood", "location", "current_project"]
similarity_floor = 0.6   # tension_candidates(): how close in meaning two facts must be
per_pass         = 4     # how many pairs to offer at once
```
...
| `MNEMNET_TENSION_SIMILARITY` | `0.6` |
| `MNEMNET_TENSION_PER_PASS` | `4` |
...
├── Provenance       — perceived / told / inferred / wanted, and one correction path
