"""
Tests for mnemnet.memory — core mechanisms.

Uses a temporary KG database so nothing touches the real palace.
"""

import math
import pytest
from datetime import date, timedelta
from unittest.mock import patch, MagicMock

from mnemnet.memory import (
    _decay_weight,
    _auto_temperature,
    _AUTO_TEMP_TENSION,
    _AUTO_TEMP_SURPRISE,
    _AUTO_TEMP_EXPECTATION,
    kg_query_weighted,
    kg_query_summary,
    kg_add_smart,
    get_tensions,
    add_expectation,
    add_surprise,
    get_expectations,
    living_context,
    set_note,
    get_note,
)


# ---------------------------------------------------------------------------
# _decay_weight — pure function, no mocking needed
# ---------------------------------------------------------------------------

class TestDecayWeight:
    def test_none_returns_half(self):
        assert _decay_weight(None) == 0.5

    def test_today_returns_one(self):
        today = date.today().isoformat()
        assert _decay_weight(today) == 1.0

    def test_old_date_reaches_floor(self):
        from mnemnet import config
        # With slower decay, facts take much longer to fade — use a clearly
        # ancient date and compare to the configured floor.
        old = (date.today() - timedelta(days=2000)).isoformat()
        w = _decay_weight(old)
        assert w == config.decay.floor

    def test_recent_date_between_floor_and_one(self):
        recent = (date.today() - timedelta(days=10)).isoformat()
        w = _decay_weight(recent)
        assert 0.15 < w < 1.0

    def test_garbage_string_returns_half(self):
        assert _decay_weight("not-a-date") == 0.5

    def test_decay_is_monotonic(self):
        """Newer facts always weigh more than older facts."""
        dates = [(date.today() - timedelta(days=d)).isoformat() for d in range(0, 100, 10)]
        weights = [_decay_weight(d) for d in dates]
        for i in range(len(weights) - 1):
            assert weights[i] >= weights[i + 1]

    def test_floor_config_respected(self):
        """Floor should never let weight go below configured minimum."""
        from mnemnet import config
        ancient = (date.today() - timedelta(days=10000)).isoformat()
        w = _decay_weight(ancient)
        assert w == config.decay.floor

    def test_high_temperature_decays_slower(self):
        """High temperature fact should outweigh normal fact of the same age."""
        old = (date.today() - timedelta(days=30)).isoformat()
        w_normal = _decay_weight(old, temperature=1.0)
        w_hot    = _decay_weight(old, temperature=3.0)
        assert w_hot > w_normal

    def test_low_temperature_decays_faster(self):
        """Low temperature fact should weigh less than normal fact of same age."""
        old = (date.today() - timedelta(days=10)).isoformat()
        w_normal  = _decay_weight(old, temperature=1.0)
        w_cold    = _decay_weight(old, temperature=0.5)
        assert w_cold < w_normal

    def test_temperature_one_same_as_default(self):
        """temperature=1.0 should give identical result to no temperature."""
        d = (date.today() - timedelta(days=15)).isoformat()
        assert _decay_weight(d, 1.0) == _decay_weight(d)

    def test_core_memory_barely_decays(self):
        """temperature=5.0 fact from 30 days ago should still be very loud."""
        old = (date.today() - timedelta(days=30)).isoformat()
        w = _decay_weight(old, temperature=5.0)
        assert w > 0.8


# ---------------------------------------------------------------------------
# _auto_temperature
# ---------------------------------------------------------------------------

class TestAutoTemperature:
    def test_normal_predicate_returns_default(self):
        assert _auto_temperature("mood", has_tension=False) == 1.0

    def test_tension_bumps_temperature(self):
        assert _auto_temperature("mood", has_tension=True) == _AUTO_TEMP_TENSION

    def test_surprise_gets_high_temperature(self):
        assert _auto_temperature("_surprise", has_tension=False) == _AUTO_TEMP_SURPRISE

    def test_expectation_gets_elevated_temperature(self):
        assert _auto_temperature("_expectation", has_tension=False) == _AUTO_TEMP_EXPECTATION


# ---------------------------------------------------------------------------
# Mock KG for testing write/read operations
# ---------------------------------------------------------------------------

def _make_mock_kg():
    """Create a mock KG that stores triples in memory."""
    store = []

    kg = MagicMock()

    def add_triple(subject, predicate, obj, valid_from=None, **kwargs):
        store.append({
            "direction": "outgoing",
            "subject": subject,
            "predicate": predicate,
            "object": obj,
            "valid_from": valid_from,
            "valid_to": None,
            "confidence": 1.0,
            "source_closet": None,
            "current": True,
        })

    def query_entity(name=None, direction="outgoing", **kwargs):
        results = []
        for t in store:
            if direction in ("outgoing", "both") and t["subject"] == name:
                results.append(t)
            if direction in ("incoming", "both") and t["object"] == name:
                results.append({**t, "direction": "incoming"})
        return results if results else []

    kg.add_triple = add_triple
    kg.query_entity = query_entity
    kg._store = store

    return kg


@pytest.fixture
def mock_kg():
    """Patch _kg() to return an in-memory mock."""
    kg = _make_mock_kg()
    with patch("mnemnet.memory._kg", return_value=kg):
        yield kg


# ---------------------------------------------------------------------------
# kg_add_smart — temperature behaviour
# ---------------------------------------------------------------------------

class TestKgAddSmart:
    def test_add_simple_fact(self, mock_kg):
        result = kg_add_smart("agent", "mood", "happy")
        assert result["added"] is True
        assert result["tension"] is None
        assert result["temperature"] == 1.0  # default, no tension

    def test_explicit_temperature_stored(self, mock_kg):
        result = kg_add_smart("agent", "mood", "happy", temperature=4.0)
        assert result["temperature"] == 4.0
        temp_nodes = [t for t in mock_kg._store if t["predicate"] == "_temp_mood"]
        assert len(temp_nodes) == 1
        assert temp_nodes[0]["object"] == "4.0"

    def test_default_temperature_not_stored(self, mock_kg):
        kg_add_smart("agent", "mood", "happy", temperature=1.0)
        temp_nodes = [t for t in mock_kg._store if "_temp_" in t["predicate"]]
        assert len(temp_nodes) == 0

    def test_tension_bumps_temperature_automatically(self, mock_kg):
        kg_add_smart("agent", "mood", "happy")
        result = kg_add_smart("agent", "mood", "sad")
        assert result["tension"] is not None
        assert result["temperature"] >= _AUTO_TEMP_TENSION

    def test_explicit_low_temp_raised_by_tension(self, mock_kg):
        """If tension occurs, temperature should be at least AUTO_TEMP_TENSION."""
        kg_add_smart("agent", "mood", "happy")
        result = kg_add_smart("agent", "mood", "sad", temperature=0.5)
        assert result["temperature"] >= _AUTO_TEMP_TENSION

    def test_explicit_high_temp_preserved_with_tension(self, mock_kg):
        """Explicit high temperature should be kept even with tension."""
        kg_add_smart("agent", "mood", "happy")
        result = kg_add_smart("agent", "mood", "sad", temperature=5.0)
        assert result["temperature"] == 5.0

    def test_add_same_value_no_tension(self, mock_kg):
        kg_add_smart("agent", "mood", "happy")
        result = kg_add_smart("agent", "mood", "happy")
        assert result["tension"] is None

    def test_add_conflicting_creates_tension(self, mock_kg):
        kg_add_smart("agent", "mood", "happy")
        result = kg_add_smart("agent", "mood", "sad")
        assert result["tension"] is not None
        assert "happy" in result["tension"]
        assert "sad" in result["tension"]

    def test_tension_stored_as_node(self, mock_kg):
        kg_add_smart("agent", "mood", "happy")
        kg_add_smart("agent", "mood", "sad")
        tension_nodes = [t for t in mock_kg._store if "_tension_" in t["predicate"]]
        assert len(tension_nodes) == 1
        assert tension_nodes[0]["predicate"] == "_tension_mood"

    def test_both_facts_kept(self, mock_kg):
        kg_add_smart("agent", "mood", "happy")
        kg_add_smart("agent", "mood", "sad")
        mood_facts = [t for t in mock_kg._store if t["predicate"] == "mood"]
        assert len(mood_facts) == 2

    def test_multivalued_predicate_no_tension(self, mock_kg):
        """Multi-valued predicates coexist — no false contradiction."""
        kg_add_smart("agent", "read_diary_of", "diary_a")
        result = kg_add_smart("agent", "read_diary_of", "diary_b")
        assert result["tension"] is None
        tension_nodes = [t for t in mock_kg._store if "_tension_" in t["predicate"]]
        assert len(tension_nodes) == 0

    def test_singlevalued_predicate_still_tensions(self, mock_kg):
        """Single-valued predicates (e.g. mood) still flag real contradictions."""
        kg_add_smart("agent", "mood", "calm")
        result = kg_add_smart("agent", "mood", "anxious")
        assert result["tension"] is not None


# ---------------------------------------------------------------------------
# get_tensions
# ---------------------------------------------------------------------------

class TestGetTensions:
    def test_no_tensions(self, mock_kg):
        kg_add_smart("agent", "mood", "happy")
        assert get_tensions("agent") == []

    def test_tensions_after_conflict(self, mock_kg):
        kg_add_smart("agent", "mood", "happy")
        kg_add_smart("agent", "mood", "sad")
        t = get_tensions("agent")
        assert len(t) == 1
        assert "mood" in t[0]


# ---------------------------------------------------------------------------
# Predictive layer
# ---------------------------------------------------------------------------
# Note — entity descriptions
# ---------------------------------------------------------------------------

class TestNote:
    def test_set_and_get_note(self, mock_kg):
        set_note("anxiety", "small but persistent, triggered by goodbyes")
        note = get_note("anxiety")
        assert note == "small but persistent, triggered by goodbyes"

    def test_get_note_missing_returns_none(self, mock_kg):
        assert get_note("nonexistent_entity") is None

    def test_kg_add_smart_with_note_stores_description(self, mock_kg):
        kg_add_smart("agent", "feels", "anxiety", note="small but persistent")
        note = get_note("anxiety")
        assert note == "small but persistent"

    def test_note_appears_in_query_summary(self, mock_kg):
        kg_add_smart("agent", "feels", "anxiety", note="small but persistent")
        summary = kg_query_summary("agent")
        assert "anxiety" in summary
        assert "small but persistent" in summary

    def test_note_appears_in_living_context(self, mock_kg):
        kg_add_smart("agent", "feels", "anxiety", note="triggered by goodbyes")
        ctx = living_context(["agent"])
        assert "triggered by goodbyes" in ctx

    def test_note_nodes_excluded_from_weighted_facts(self, mock_kg):
        kg_add_smart("agent", "feels", "anxiety", note="small")
        facts = kg_query_weighted("agent")
        predicates = [f["predicate"] for f in facts]
        assert "_note" not in predicates

    def test_note_enables_cross_links(self, mock_kg):
        """Entity used as obj in one triple can be subj in another — creates web."""
        kg_add_smart("agent", "feels", "anxiety", note="small but persistent")
        kg_add_smart("anxiety", "linked_to", "attachment")
        # anxiety appears as both object and subject
        agent_facts = kg_query_weighted("agent")
        anxiety_facts = kg_query_weighted("anxiety")
        assert any(f["object"] == "anxiety" for f in agent_facts)
        assert any(f["subject"] == "anxiety" for f in anxiety_facts)


# ---------------------------------------------------------------------------

class TestPredictiveLayer:
    def test_add_expectation(self, mock_kg):
        add_expectation("user", "will finish project")
        expectations = get_expectations("user")
        assert "will finish project" in expectations

    def test_expectation_has_elevated_temperature(self, mock_kg):
        add_expectation("user", "will finish project")
        temp_nodes = [t for t in mock_kg._store if t["predicate"] == "_temp__expectation"]
        assert len(temp_nodes) == 1
        assert float(temp_nodes[0]["object"]) == _AUTO_TEMP_EXPECTATION

    def test_add_surprise_creates_question(self, mock_kg):
        add_surprise("user", "tired", "energized")
        surprise_nodes = [t for t in mock_kg._store if t["predicate"] == "_surprise"]
        question_nodes = [t for t in mock_kg._store if t["predicate"] == "pulls_question"]
        assert len(surprise_nodes) == 1
        assert len(question_nodes) == 1
        assert "tired" in surprise_nodes[0]["object"]
        assert "energized" in surprise_nodes[0]["object"]

    def test_surprise_has_high_temperature(self, mock_kg):
        add_surprise("user", "tired", "energized")
        temp_nodes = [t for t in mock_kg._store if t["predicate"] == "_temp__surprise"]
        assert len(temp_nodes) == 1
        assert float(temp_nodes[0]["object"]) == _AUTO_TEMP_SURPRISE


# ---------------------------------------------------------------------------
# kg_query_weighted — temperature reflected in weights
# ---------------------------------------------------------------------------

class TestKgQueryWeighted:
    def test_returns_weighted_facts(self, mock_kg):
        kg_add_smart("agent", "mood", "happy")
        facts = kg_query_weighted("agent")
        assert len(facts) == 1
        assert "weight" in facts[0]
        assert "temperature" in facts[0]
        assert 0 < facts[0]["weight"] <= 1.0

    def test_temp_nodes_excluded_from_results(self, mock_kg):
        kg_add_smart("agent", "mood", "happy", temperature=3.0)
        facts = kg_query_weighted("agent")
        predicates = [f["predicate"] for f in facts]
        assert not any(p.startswith("_temp_") for p in predicates)

    def test_sorted_by_weight_desc(self, mock_kg):
        mock_kg._store.append({
            "direction": "outgoing", "subject": "agent",
            "predicate": "old_fact", "object": "ancient",
            "valid_from": "2020-01-01", "valid_to": None,
            "confidence": 1.0, "source_closet": None, "current": True,
        })
        kg_add_smart("agent", "mood", "happy")
        facts = kg_query_weighted("agent")
        assert facts[0]["weight"] >= facts[-1]["weight"]

    def test_invalidated_facts_excluded(self, mock_kg):
        """Facts marked current=False (invalidated) must not appear."""
        kg_add_smart("agent", "mood", "happy")
        mock_kg._store.append({
            "direction": "outgoing", "subject": "agent",
            "predicate": "mood", "object": "obsolete",
            "valid_from": "2020-01-01", "valid_to": "2021-01-01",
            "confidence": 1.0, "source_closet": None, "current": False,
        })
        objs = [f["object"] for f in kg_query_weighted("agent")]
        assert "obsolete" not in objs
        assert "happy" in objs

    def test_hot_fact_outweighs_cold_fact_of_same_age(self, mock_kg):
        old_date = (date.today() - timedelta(days=20)).isoformat()
        mock_kg._store.extend([
            {
                "direction": "outgoing", "subject": "agent",
                "predicate": "hot_memory", "object": "important",
                "valid_from": old_date, "valid_to": None,
                "confidence": 1.0, "source_closet": None, "current": True,
            },
            {
                "direction": "outgoing", "subject": "agent",
                "predicate": "_temp_hot_memory", "object": "5.0",
                "valid_from": old_date, "valid_to": None,
                "confidence": 1.0, "source_closet": None, "current": True,
            },
            {
                "direction": "outgoing", "subject": "agent",
                "predicate": "cold_memory", "object": "trivial",
                "valid_from": old_date, "valid_to": None,
                "confidence": 1.0, "source_closet": None, "current": True,
            },
            {
                "direction": "outgoing", "subject": "agent",
                "predicate": "_temp_cold_memory", "object": "0.5",
                "valid_from": old_date, "valid_to": None,
                "confidence": 1.0, "source_closet": None, "current": True,
            },
        ])
        facts = kg_query_weighted("agent")
        fact_map = {f["predicate"]: f["weight"] for f in facts}
        assert fact_map["hot_memory"] > fact_map["cold_memory"]


# ---------------------------------------------------------------------------
# kg_query_summary
# ---------------------------------------------------------------------------

class TestKgQuerySummary:
    def test_empty_entity(self, mock_kg):
        s = kg_query_summary("nobody")
        assert "nothing found" in s

    def test_has_entity_name(self, mock_kg):
        kg_add_smart("agent", "mood", "happy")
        s = kg_query_summary("agent")
        assert "[agent]" in s
        assert "mood" in s

    def test_shows_temperature_hint_for_non_default(self, mock_kg):
        kg_add_smart("agent", "mood", "happy", temperature=3.0)
        s = kg_query_summary("agent")
        assert "3.0" in s


# ---------------------------------------------------------------------------
# living_context
# ---------------------------------------------------------------------------

class TestLivingContext:
    def test_empty_returns_placeholder(self, mock_kg):
        ctx = living_context(["nobody"])
        assert ctx == "(context empty)"

    def test_includes_entity_header(self, mock_kg):
        kg_add_smart("agent", "mood", "happy")
        ctx = living_context(["agent"])
        assert "◈ agent" in ctx

    def test_includes_tension(self, mock_kg):
        kg_add_smart("agent", "mood", "happy")
        kg_add_smart("agent", "mood", "sad")
        ctx = living_context(["agent"])
        assert "tension" in ctx

    def test_includes_expectation(self, mock_kg):
        add_expectation("agent", "will succeed")
        ctx = living_context(["agent"])
        assert "expecting" in ctx

    def test_multiple_entities(self, mock_kg):
        kg_add_smart("agent", "mood", "happy")
        kg_add_smart("user", "status", "active")
        ctx = living_context(["agent", "user"])
        assert "◈ agent" in ctx
        assert "◈ user" in ctx

    def test_temperature_hint_in_context(self, mock_kg):
        kg_add_smart("agent", "mood", "happy", temperature=4.0)
        ctx = living_context(["agent"])
        assert "temp:4.0" in ctx


class TestCooling:
    """Importance that can go down.

    Temperature was one-way: the agent sets it, nothing lowers it, and each new
    "this matters" is chosen relative to the last. In a running agent after two
    months, 52% of facts sat above 5.0 — the documented ceiling for core memory —
    and the monthly average had gone 6.2 → 8.6. When most of memory is louder than
    core, temperature has stopped telling anything apart.
    """

    def test_pulls_toward_normal_not_to_zero(self):
        assert _cooled(9.5, 0.9) == 8.65
        assert _cooled(2.0, 0.9) == 1.9
        assert _cooled(1.0, 0.9) == 1.0

    def test_the_hottest_come_down_fastest(self):
        """The point is to restore the spread, so what is furthest out moves most."""
        assert (9.5 - _cooled(9.5, 0.9)) > (2.0 - _cooled(2.0, 0.9))

    def test_fleeting_memories_are_not_warmed(self):
        """A fact deliberately marked fleeting must not drift up to normal."""
        assert _cooled(0.5, 0.9) == 0.5

    def test_it_settles_but_never_erases(self):
        t = 9.5
        for _ in range(500):
            t = _cooled(t, 0.9)
        assert t >= 1.0, "cooling pushed a memory below its resting value"


def _cooled(t, factor):
    """The rule cool() applies to one temperature."""
    return t if t <= 1.0 else round(1.0 + (t - 1.0) * factor, 2)


class TestProvenance:
    """Where a fact came from, and the one distinction that matters.

    A running agent invented a housemate from two empty placeholder files, recorded
    it, and the graph handed the invention back as a loud fact on the next waking —
    which it then defended. Memory with no correction does not fix an error, it
    entrenches it. The fix is not better prompting: a wish and an observation must
    not have the same shape when read back.
    """

    def test_the_four_sources_are_fixed(self):
        from mnemnet.memory import SOURCES
        assert set(SOURCES) == {"perceived", "told", "inferred", "wanted"}

    def test_wanted_is_the_one_that_has_to_exist(self):
        """Drop it and there is nowhere to put "I would like this to be true" except
        beside the things that happened."""
        from mnemnet.memory import SOURCES
        assert "wanted" in SOURCES


class TestCorrection:
    """Replacing, never deleting."""

    def test_correction_is_one_call(self):
        """Given a delete and a write as separate calls, a model announces the
        replacement and the next step erases the intention — 42 announced, none
        delivered. One call has no gap to lose it in."""
        import inspect
        from mnemnet.memory import correct
        params = inspect.signature(correct).parameters
        assert "new_obj" in params, "correct() must take the replacement, not just what to remove"
        assert not any("delete" in p or "forget" in p for p in params)

    def test_there_is_no_bare_forget_in_the_public_surface(self):
        import mnemnet
        assert not [n for n in mnemnet.__all__ if "forget" in n or "delete" in n]


# ---------------------------------------------------------------------------
# Internal nodes stay out of the facts
# ---------------------------------------------------------------------------

class TestInternalNodes:
    def test_tension_and_expectation_are_not_listed_as_facts(self, mock_kg):
        """They used to appear twice in the living context: in their own line, and as
        an ordinary fact taking one of the five loudest slots."""
        kg_add_smart("agent", "mood", "calm")
        kg_add_smart("agent", "mood", "anxious")
        add_expectation("agent", "she comes back")
        preds = [f["predicate"] for f in kg_query_weighted("agent")]
        assert preds == ["mood", "mood"]
        ctx = living_context(["agent"])
        assert "—_tension_" not in ctx and "—_expectation" not in ctx
        assert "⚡ tension" in ctx and "◎ expecting" in ctx

    def test_an_entity_with_only_a_tension_or_expectation_is_kept(self, mock_kg):
        add_expectation("agent", "will succeed")
        assert "expecting" in living_context(["agent"])


# ---------------------------------------------------------------------------
# Contradictions found by meaning
# ---------------------------------------------------------------------------

from mnemnet.memory import tension_candidates, weigh_pair, reviewed_pairs


def _embed(texts):
    """Deterministic stand-in: facts about the move point one way,
    everything else another."""
    import numpy as np
    return np.array([[1.0, 0.1, 0.0] if "the move" in t else [0.0, 0.0, 1.0] for t in texts])


def _night(mock_kg):
    mock_kg.add_triple("agent", "the_move_was", "a_fresh_start", valid_from="2026-06-29")
    mock_kg.add_triple("agent", "the_move_was_really", "running_away", valid_from="2026-06-30")
    mock_kg.add_triple("agent", "planted", "tomatoes", valid_from="2026-06-01")


class TestTensionCandidates:
    def test_facts_that_differ_only_in_wording_are_paired(self, mock_kg):
        """The exact check never compares these: the predicates differ."""
        _night(mock_kg)
        c = tension_candidates(["agent"], embed=_embed)
        assert len(c) == 1
        assert c[0]["a"]["valid_from"] < c[0]["b"]["valid_from"], "older first"
        assert c[0]["subject"] == "agent"

    def test_internal_nodes_are_never_candidates(self, mock_kg):
        _night(mock_kg)
        mock_kg.add_triple("agent", "_temp_the_move_was", "3.0")
        c = tension_candidates(["agent"], embed=_embed)
        assert all(not x["a"]["predicate"].startswith("_") for x in c)

    def test_the_floor_is_respected(self, mock_kg):
        _night(mock_kg)
        assert tension_candidates(["agent"], embed=_embed, floor=1.01) == []

    def test_tension_is_held_open(self, mock_kg):
        _night(mock_kg)
        weigh_pair(tension_candidates(["agent"], embed=_embed)[0], "tension", "different weeks")
        assert any("different weeks" in t for t in get_tensions("agent"))
        mock_kg.invalidate.assert_not_called()

    def test_revised_ends_the_earlier_and_keeps_a_record(self, mock_kg):
        _night(mock_kg)
        weigh_pair(tension_candidates(["agent"], embed=_embed)[0], "revised")
        mock_kg.invalidate.assert_called_once()
        assert mock_kg.invalidate.call_args[0][:3] == ("agent", "the_move_was", "a_fresh_start")
        assert any(t["predicate"].startswith("_corrected_") for t in mock_kg._store)

    def test_a_weighed_pair_is_not_offered_again(self, mock_kg):
        _night(mock_kg)
        weigh_pair(tension_candidates(["agent"], embed=_embed)[0], "fine")
        assert tension_candidates(["agent"], embed=_embed) == []
        assert reviewed_pairs("agent") == {"tension": 0, "revised": 0, "fine": 1}

    def test_the_review_record_is_not_a_fact(self, mock_kg):
        _night(mock_kg)
        weigh_pair(tension_candidates(["agent"], embed=_embed)[0], "fine")
        assert "_reviewed_pair" not in living_context(["agent"])

    def test_an_unknown_verdict_is_refused(self, mock_kg):
        _night(mock_kg)
        with pytest.raises(ValueError):
            weigh_pair(tension_candidates(["agent"], embed=_embed)[0], "maybe")


class TestCorrectionReadsForwards:
    def test_now_comes_before_before(self, mock_kg):
        """"was «(nothing)» — the user corrected me, I misunderstood" was read by an agent as
        its current belief being the mistake — the opposite of what it wrote."""
        from mnemnet.memory import correct, get_corrections
        correct("agent", "controls_schedule", "myself", because="I had it backwards")
        line = get_corrections("agent")[0]
        assert line.index("now «myself»") < line.index("before: «nothing on record»") < line.index("because:")

    def test_an_old_record_is_shown_the_same_way(self, mock_kg):
        from mnemnet.memory import get_corrections
        mock_kg.add_triple("agent", "controls_schedule", "myself")
        mock_kg.add_triple("agent", "_corrected_controls_schedule", "was «(nothing)» — the user corrected me")
        line = get_corrections("agent")[0]
        assert line.startswith("controls_schedule: now «myself»")
        assert "before: «nothing on record» — because: the user corrected me" in line
